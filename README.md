...
| **언어** | C Language |
| **주요 부품** | ROBIT 실습보드, 8-Bit LED, 스위치 4개 |
...
### 주요 회로 특징
* **LED:** 애노드가 5V, 캐소드가 PA 핀에 연결된 Active Low 구조. `0`을 출력해야 점등됨
* **스위치:** 5V - 10kΩ - 핀 - 스위치 - GND 의 외부 풀업 구조. 눌림 = LOW
* **디바운스:** 각 스위치에 104(0.1µF) 커패시터가 병렬 연결됨. RC 시정수 약 1ms
...
sei();
```
`ISCn0`은 리셋 초기값이 0이므로 별도로 건드리지 않음. 비트 위치를 숫자로 직접 쓰면 어느 INT를 설정하는지 혼동되기 쉬워 `ISC31` 등 이름 상수를 사용함.
...
```
입력값은 반드시 `PINC`로 읽어야 함. 내부 풀업을 켠 상태에서 `PORTC`로 읽으면 항상 1이 반환되어 스위치 상태를 알 수 없음.
...
```
보드의 물리적 배치가 PA0을 왼쪽 끝, PA7을 오른쪽 끝에 두고 있어 비트 시프트 방향과 화면상 방향이 반대임. 따라서 좌측 이동에 `>>`, 우측 이동에 `<<`를 사용함. `EIFR`은 0이 아닌 1을 써야 플래그가 지워짐.
...
### 검증 과정에서 확인한 사항
초기 구현에서 이동 방향이 요구사항과 반대로 동작함. 코드상 `<<` 연산이 실제 보드에서는 우측 이동으로 나타났으며, 원인은 LED의 물리적 배치 순서임. 회로도만으로는 판단할 수 없어 실제 보드 동작을 확인한 뒤 방향을 수정함.
